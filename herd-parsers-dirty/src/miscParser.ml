...
let dump_location = function
  ...
  | Location_global v -> SymbConstant.show_v v
  | Location_deref (v,i) ->
      Printf.sprintf "%s[%i]" (SymbConstant.show_v v) i

let dump_rval loc = match loc with
  | Location_reg (i,r) -> Printf.sprintf "%i:%s" i r
  | Location_sreg s -> Misc.dump_symbolic s
  | Location_global v -> Printf.sprintf "*%s" (SymbConstant.show_v v)
...
let pp_atom (loc,v) =
  sprintf "%s=%s" (dump_location loc) (SymbConstant.show_v v)
